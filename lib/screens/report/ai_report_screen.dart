...
class AiReportScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // 🌟 [수정] Scaffold로 감싸고 AppBar 추가
    return Scaffold(
      backgroundColor: const Color(0xFFFBF9FF), // from-pink-50 to-purple-50 유사색
      // 🌟 [추가] 홈 화면에서 Push로 진입 시 뒤로가기 버튼이 자동 생성되는 AppBar
      appBar: AppBar(
        title: const Text('AI 리포트'),
        backgroundColor: Colors.white,
        foregroundColor: AuraColors.gray800,
        elevation: 1,
      ),
  ...
  Widget buildHeader() {
    return Padding(
      padding: const EdgeInsets.only(left: 24, right: 24, top: 24, bottom: 16), // 🌟 AppBar가 생겼으므로 top padding 수정
      child: Row(
